...
UI building blocks:
- **Map:** the canonical **equirectangular hex map** (`xos-world-map.png`, generated by
  `scripts/generate_map.py`). Hexes are the atomic tiles; the ~49 regions are hex clusters
  (geometry from `02`). Region fill = controlling polity; hover = stats; Realtime re-color
  on tick. The generator is the offline source of truth for tile coordinates the DB seeds
  from.
